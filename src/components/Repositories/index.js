...
class Repositories extends Component {
  ...
  getRepositoriesList = async () => {
    ...
    if (username === undefined) {
      ...
      if (response.ok === true) {
        const getBranch = async reposName => {
          const branchUrl = `https://api.github.com/repos/${username}/${reposName}/branches`
          ...
          if (branchResponse.ok === true) {
            return branchData
          }
          return this.onFailureResponse()
        }

        const formatReposList = async () => {
          const formatedReposList = []
          for (const eachItem of data) {
            const branches = await getBranch(eachItem.name)

            formatedReposList.push({
              id: eachItem.id,
              name: eachItem.name,
          ...
          this.onSuccessFulResponse(formatedReposList)
        }
        formatReposList()
      } else {
